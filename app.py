...
def apply_simulasi_fiskal_to_pdb_nominal(pdb_df: pd.DataFrame, simulasi_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Revisi pembagi terbaru: Q1=1.82, Q2=1.86, Q3=1.88, Q4=1.91
    ...
    pkp_mask = work["indikator"].astype(str).str.strip() == "PKP"
    ...
    for col, div in divisors.items():
        ...
        base_pkp = pd.to_numeric(work.loc[pkp_mask, col], errors="coerce").fillna(0.0)
        work.loc[pkp_mask, col] = base_pkp + addition

        # Karena hanya PKP yang berubah, PDB Aggregate bertambah sebesar addition yang sama.
        if agg_mask.any():
