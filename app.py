...
MACRO_TAX_IMPACT_RULES = {
    # perubahan +0,1 dari APBN 2026 => tambahan dampak pada Penerimaan Perpajakan
    "Pertumbuhan ekonomi (%)": 2080.30,
    "Inflasi (%)": 1862.99,
}
...
def calculate_macro_tax_impacts(simulasi_makro_df: Optional[pd.DataFrame]) -> dict:
    impacts = {name: 0.0 for name in MACRO_TAX_IMPACT_RULES}
    ...
    for indikator, coefficient in MACRO_TAX_IMPACT_RULES.items():
        row = work.loc[work["indikator"] == indikator]
        if row.empty:
            impacts[indikator] = 0.0
            continue
        ...
        if pd.isna(apbn_val) or pd.isna(shock_val):
            impacts[indikator] = 0.0
            continue
        # setiap perubahan 0,1 terhadap APBN 2026 memengaruhi PP sebesar coefficient
        delta = float(shock_val) - float(apbn_val)
        impacts[indikator] = round((delta / 0.1) * coefficient, 2)

    return impacts


def calculate_total_tax_impact(simulasi_makro_df: Optional[pd.DataFrame]) -> float:
    impacts = calculate_macro_tax_impacts(simulasi_makro_df)
    return round(sum(impacts.values()), 2)
...
def render_simulasi_makro_editor() -> pd.DataFrame:
    ...
    st.caption(
        ...
        "Mulai versi ini, perubahan pada Pertumbuhan ekonomi (%) dan Inflasi (%) "
        "langsung mengubah Dampak pada Penerimaan Perpajakan di Blok Fiskal."
    )
    ...
    impact_detail = calculate_macro_tax_impacts(edited_df)
    st.info(
        "Aturan dampak pajak aktif: "
        f"Pertumbuhan ekonomi ±0,1 => ±{fmt_fiskal_number(MACRO_TAX_IMPACT_RULES['Pertumbuhan ekonomi (%)'], 2)}; "
        f"Inflasi ±0,1 => ±{fmt_fiskal_number(MACRO_TAX_IMPACT_RULES['Inflasi (%)'], 2)}. "
        f"Preview draft dampak total saat ini: {fmt_fiskal_number(sum(impact_detail.values()), 2)}."
    )
...
def render_fiskal_block_table(simulasi_makro_df: Optional[pd.DataFrame] = None):
    macro_impacts = calculate_macro_tax_impacts(simulasi_makro_df)
    d_pp = round(sum(macro_impacts.values()), 2)
    ...
    if macro_impacts:
        detail_lines = []
        for name, val in macro_impacts.items():
            detail_lines.append(f"- {name}: {fmt_fiskal_number(val, 2)}")
        st.caption(
            "Rincian pembentuk Dampak pada '1. Penerimaan Perpajakan':\n" + "\n".join(detail_lines)
        )
...
st.caption(
    ...
    "Input asumsi shock makro sudah dihubungkan ke Blok Fiskal untuk menghitung dampak pada penerimaan perpajakan."
)
