...
st.markdown(
    ...
    <style>
        ...
        .fiskal-table th:first-child,
        .fiskal-table td:first-child {
            text-align: left;
        }
        .fiskal-table th {
            background: #F3F4F6;
...
def _build_growth_tables_from_wide(wide: pd.DataFrame):
    ...
    for comp in PDB_COMPONENTS:
        ...
        for q in [1, 2, 3, 4]:
            ...
            yoy_row[date_map[q]] = (
                float(sel["yoy"].iloc[-1])
                if not sel.empty and pd.notna(sel["yoy"].iloc[-1])
                else None
            )
            qtq_row[date_map[q]] = (
                float(sel["qtq"].iloc[-1])
                if not sel.empty and pd.notna(sel["qtq"].iloc[-1])
                else None
            )
...
def calculate_pertumbuhan_ekonomi_tax_impact(simulasi_makro_df: Optional[pd.DataFrame]) -> Optional[float]:
    ...
    required_cols = {"indikator", "apbn_2026", "shock"}
    if not required_cols.issubset(simulasi_makro_df.columns):
        return None

    work = simulasi_makro_df.copy()
    work["indikator"] = work["indikator"].astype(str).str.strip()
    row = work.loc[work["indikator"] == "Pertumbuhan ekonomi (%)"]
    if row.empty:
        return None
    ...
    dampak = ((float(shock_val) - float(apbn_val)) / 0.1) * 2080.30
...
def render_simulasi_fiskal_editor() -> pd.DataFrame:
    ...
    st.caption(
        ...
        if has_pending
        else "Draft simulasi fiskal sudah sinkron dengan Tabel Utama."
    )
...
def render_simulasi_makro_editor() -> pd.DataFrame:
    ...
    st.caption(
        ...
        "Pada versi ini nilai shock disimpan di session state dan siap dihubungkan ke formula simulasi berikutnya."
    )
    ...
    if c1.button("Terapkan Shock Makro", use_container_width=True, type="primary"):
        ...
        st.session_state["simulasi_makro_notice"] = (
            "success",
            "Input shock makro berhasil disimpan.",
        )
        st.rerun()
    ...
    st.caption(
        ...
        if has_pending
        else "Input shock makro sudah sinkron."
    )
...
def render_fiskal_block_table(simulasi_makro_df: Optional[pd.DataFrame] = None):
    dampak_pertumbuhan_ekonomi = calculate_pertumbuhan_ekonomi_tax_impact(simulasi_makro_df)

    fiskal_rows = [
        {"uraian": "A. Pendapatan Negara dan Hibah", "apbn": 3153580, "outlook": None, "dampak": None, "bold": True},
        {"uraian": "1. Penerimaan Perpajakan", "apbn": 2693714, "outlook": None, "dampak": dampak_pertumbuhan_ekonomi, "bold": False},
        {"uraian": "2. Penerimaan Negara Bukan Pajak", "apbn": 459200, "outlook": None, "dampak": None, "bold": False},
        {"uraian": "3. Hibah", "apbn": 666, "outlook": None, "dampak": None, "bold": False},
        {"uraian": "B. Belanja Negara", "apbn": 3842728, "outlook": None, "dampak": None, "bold": True},
        {"uraian": "1. Belanja Pemerintah Pusat", "apbn": 3149733, "outlook": None, "dampak": None, "bold": False},
        {"uraian": "2. Transfer ke Daerah", "apbn": 692995, "outlook": None, "dampak": None, "bold": False},
        {"uraian": "C. Surplus/Defisit", "apbn": -689148, "outlook": None, "dampak": None, "bold": True},
        {"uraian": "D. Pembiayaan Anggaran", "apbn": 689148, "outlook": None, "dampak": None, "bold": True},
    ]
    ...
    for row in fiskal_rows:
        ...
        body_rows.append(
            ...
            f"<td style='{fw}'>{fmt_apbn_image(row['apbn'])}</td>"
            f"<td style='{fw}'>{fmt_fiskal_dampak(row['dampak'])}</td>"
            f"<td style='{fw}'>{fmt_apbn_image(row['outlook'])}</td>"
            f"</tr>"
        )

    fiskal_html = (
        "<table class='fiskal-table'>"
        "<thead><tr><th>Uraian</th><th>APBN 2026</th><th>Dampak</th><th>Outlook</th></tr></thead>"
        f"<tbody>{''.join(body_rows)}</tbody></table>"
    )
    st.markdown(fiskal_html, unsafe_allow_html=True)
    st.caption(
        "Format tabel fiskal menampilkan kolom APBN 2026, Dampak, dan Outlook, "
        "dengan struktur A/B/C/D dan sub-uraian. Baris '1. Penerimaan Perpajakan' kini otomatis "
        "menghitung dampak dari shock pada indikator 'Pertumbuhan ekonomi (%)' dengan sensitivitas "
        "0,1 poin = 2.080,30."
    )
...
st.caption(
    ...
    "Input asumsi shock makro sudah disediakan di Blok Fiskal untuk integrasi formula pada tahap berikutnya."
)
...
with top_nominal_tab:
    render_main_comparison_table(
        ...
        note_text=(
            ...
            "Tabel input shock makro sekarang tersedia di Blok Fiskal di bawah tabel fiskal."
        ),
    )
...
with top_yoy_tab:
    render_main_comparison_table(
        ...
        formatter=fmt_pct,
        note_text=(
            "Tabel YoY memakai layout scroll horizontal. Nilai Shock Makro di tabel utama tetap mengikuti Shock Fiskal "
            "sampai formula makro dihubungkan."
        ),
    )
