...
def apply_simulasi_fiskal_to_pdb_nominal(pdb_df: pd.DataFrame, simulasi_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if simulasi_df is None or simulasi_df.empty:
        return ensure_full_year_from_quarters(pdb_df)
    ...
    for col, div in divisors.items():
        ...
        base_series = pd.to_numeric(work.loc[target_mask, col], errors="coerce").fillna(0.0)
        work.loc[target_mask, col] = base_series + addition
