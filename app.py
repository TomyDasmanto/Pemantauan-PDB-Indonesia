...
def apply_simulasi_fiskal_to_pdb_nominal(pdb_df: pd.DataFrame, simulasi_df: pd.DataFrame) -> pd.DataFrame:
    ...
    work = pdb_df.copy()
    if "indikator" not in work.columns:
        return work

    # Bantuan Pangan (dalam miliar) -> tambahan ke PKP / konsumsi pemerintah
    divisors = {"out_tw1": 180.0, "out_tw2": 184.0, "out_tw3": 185.0, "out_tw4": 191.0}
    sim_row = simulasi_df.loc[simulasi_df["indikator"].astype(str).str.strip() == "Bantuan Pangan"]
    if sim_row.empty:
        return ensure_full_year_from_quarters(work)

    target_mask = work["indikator"].astype(str).str.strip() == "PKP"
    if not target_mask.any():
        return ensure_full_year_from_quarters(work)

    for col, div in divisors.items():
        if col not in work.columns:
            work[col] = None
        work[col] = pd.to_numeric(work[col], errors="coerce")
        ...
        base_val = pd.to_numeric(work.loc[target_mask, col], errors="coerce").fillna(0.0)
        work.loc[target_mask, col] = base_val + addition
...
adjusted_pdb_nominal = apply_simulasi_fiskal_to_pdb_nominal(adjusted_pdb_nominal, simulasi_fiskal_df)
...
with tab_pdb:
    ...
    render_table_block(adjusted_pdb_nominal, block_key="pdb", chart_key="tab_pdb_nominal_2026")
    if pdb_tables is not None:
        render_growth_table(pdb_tables.get("yoy", empty_df("pdb")), "Tabel Year on Year (YoY)", header_fill=PRIMARY, chart_key="tab_pdb_yoy_table")
        render_growth_table(pdb_tables.get("qtq", empty_df("pdb")), "Tabel Quarter to Quarter (QtQ)", header_fill=PRIMARY, chart_key="tab_pdb_qtq_table")
