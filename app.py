...
# PDB Aggregate ditambahkan kembali sesuai permintaan pengguna
# dengan formula: Konsumsi RT + Konsumsi LNPRT + PKP + PMTB +
# Change in Stocks + Ekspor + Impor.
PDB_BASE_COMPONENTS = [
    "Konsumsi RT",
    "Konsumsi LNPRT",
    "PKP",
...
PDB_AGGREGATE_NAME = "PDB Aggregate"
PDB_COMPONENTS = [*PDB_BASE_COMPONENTS, PDB_AGGREGATE_NAME]
...
def dynamic_table_height(view: pd.DataFrame, row_px: int = 35, header_px: int = 38, min_px: int = 120, max_px: int = 420) -> int:
    h = header_px + row_px * len(view)
...
def derive_pdb_from_realisasi(source: Union[str, bytes]):
    ...
    mapping = {}
    for indikator in PDB_BASE_COMPONENTS:
    ...
    # Formula PDB Aggregate sesuai permintaan user: semua komponen dijumlahkan,
    # termasuk Impor dengan tanda plus.
    level_df[PDB_AGGREGATE_NAME] = (
...
def render_table_block(block_df: pd.DataFrame, block_key: str = ""):
    ...
    st.dataframe(
        ...
        height=dynamic_table_height(view),
    )
