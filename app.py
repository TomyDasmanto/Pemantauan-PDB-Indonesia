...
def render_fiskal_block_table(simulasi_makro_df: Optional[pd.DataFrame] = None):
    d_pp = calculate_pertumbuhan_ekonomi_tax_impact(simulasi_makro_df)
    d_pp = 0.0 if d_pp is None else d_pp
    ...
    for r in fiskal_rows:
        ...
        rows.append(
            f"<tr>"
            f"<td style='{fw}'>{r['uraian']}</td>"
            f"<td style='{fw}'>{fmt_apbn_image(r['apbn'])}</td>"
            f"<td style='{fw}'>{fmt_fiskal_dampak(r['dampak'])}</td>"
            f"<td style='{fw}'>{fmt_fiskal_dampak(r['outlook'])}</td>"
            f"</tr>"
        )
    ...
    st.caption("Format tabel fiskal menampilkan kolom APBN 2026, Dampak, dan Outlook, dengan struktur A/B/C/D dan sub-uraian.")
